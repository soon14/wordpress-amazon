...
<html lang="zh-CN">
<head>
    <link rel="stylesheet" type="text/css" href="<?php echo RES;?>css/wei_canyin.css?2014" media="all" />
...
width: 35px;
padding: 0;
background: url(./source/modules/shopping3/style/img/mar-close.png) no-repeat center center;
...
height: 13px;
padding: 0;
background: url(./source/modules/shopping3/style/img/mar-v.png) no-repeat center center;
...
<div class="center p40" id="box1">
	<?php  if($set['shop_notice']) { ?>
	<div class="shop-marquee" id="shop_notices">
		<div class="notice-icon"></div>
		<div class="marquee-content">
			<marquee scrollamount="2" behavior="alternate" style="">
				<?php  echo $set['shop_notice'];?>	
			</marquee>
		</div>
		<div class="ordernotice-close-icon">
		</div>
	</div>
	<?php  } ?>
    <nav id='navBar'>
...
//          alert(id);return false;
            ...
            var TPL = '<dl dunitname="{unit}" dsubcount="1" dishid="{id}" dname="{name}" dtaste="" ddescribe="{note}" dprice="{price}" dishot="{tag_name}" dspecialprice="{discount_price}" disspecial="1" people="{sellnums}" >\
					...
					<span>{label}</span><p><b>销量:{sellnums}</b>库存:{total}</p></em>\
					</dd>\
					<dd class="btn">\
...
$(document).ready(function(){
	$('#shop_notices').click(function(){
		$('#shop_notices').hide();
		$('#box1').removeClass('p40');
