...
<body id="page_intelOrder" class="intelPage">
    	    <div class="center">
    	<header>
    		<span class="pCount">共<?php  echo $_GPC['combo_nums'];?>人用餐<a href="<?php  echo  $this->createMobileUrl('wlgenius')?>">重选人数</a></span>
            <label>
                <i>共计：</i>
    ...
    <script type="text/javascript">
        ...
        function switchList(){
            ...
            var TPL = '<article><h2>{title}</h2>\
                                <dl dunitname="{unit}" dsubcount="{sellnums}"  dname="{title}" dtaste=""   dprice="{productprice}" dishot="0" dpicture="{thumb}" dspecialprice="{marketprice}" >\
                                    <dt><h3>{title}</h3></dt>\
                                    <dd>\
