...
//去掉换行和空格   public  function deletehtml($str){         $str = trim($str);         $str=strip_tags($str,"");         $str=str_replace(array("\r\n", "\r", "\n"), "", $str);         return $str;   }
	...
	public function doMobileLottery() {
		...
		if (empty($_W['fans']['from_user'])) {
			message('非法访问，请关注后在访问！');
		}
		$fromuser = $_W['fans']['from_user'];
		//$profile = fans_require($fromuser, array('realname', 'mobile', 'qq'), '需要完善资料后才能抽奖.');		$profile=fans_search($fromuser,array('realname', 'mobile'));	
		$id = intval($_GPC['id']);
		$scratchcard = pdo_fetch("SELECT id, periodlottery,picture,shareurl,description,maxlottery, rule, hitcredit, misscredit FROM ".tablename('scratchcard_reply')." WHERE rid = '$id' LIMIT 1");
		if (empty($scratchcard)) {
			message('非法访问，请重新发送消息进入抽奖页面！');
		}				$scratchcard['description']=$this->deletehtml($scratchcard['description']);
		...
		$myaward = pdo_fetchall("SELECT id, award, description, status FROM ".tablename('scratchcard_winner')." WHERE from_user = '{$fromuser}' AND aid != '0' AND award <> '' AND rid = '$id' ORDER BY createtime DESC");
	...
	public function doMobileGetAward() {
		...
		$gifts = pdo_fetchall("SELECT id, probalilty, inkind FROM ".tablename('scratchcard_award')." WHERE rid = '$id' ORDER BY probalilty ASC");
		//计算每个礼物的概率
		$probability = 0;
		$rate = 1;
		$award = array();
		...
		$result['myaward'] = pdo_fetchall("SELECT id, award, description, status FROM ".tablename('scratchcard_winner')." WHERE from_user = '{$fromuser}' AND aid != '0' AND award <> '' AND rid = '$id' ORDER BY createtime DESC");
		$mycredit = pdo_fetchcolumn("SELECT SUM(description) FROM ".tablename('scratchcard_winner')." WHERE from_user = '{$fromuser}' AND aid = '0' AND award <> '' AND rid = '$id'");
	...
	public function doMobileSetStatus() {
		...
		$fromuser = $_W['fans']['from_user'];
		$id = intval($_GPC['id']);		$reply=pdo_fetch("select * from ".tablename("scratchcard_reply")." where rid=".$id);		$mobile=$_GPC['mobile'];		$realname=$_GPC['realname'];		if(empty($mobile) || empty($realname)){			message('请输入电话号码或者名称！');		}else{	        $data = array(				'mobile' => $mobile,				'realname'=>$realname,				'status'=>2			 );			fans_update($fromuser, $data);			message($data, '', 'ajax');				}
	}
