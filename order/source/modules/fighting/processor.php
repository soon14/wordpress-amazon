...
class FightingModuleProcessor extends WeModuleProcessor {
		...
		if (!$this->inContext) {
			...
			if (!$info['nickname']) {
				...
				return $this->respNews(array (
					...
					'Description' => "点击进入登记",
					'PicUrl' => $_W['attachurl'] . $row['picture'],
					'Url' => $this->createMobileUrl('Register', array('id' => $rid)),
				));
				...
				//排名
				$text= "您今天在" . date('H:i:s', $sql_fighting['lasttime']) . "已经答过题啦，明天再来吧！" . $ripaiming;
				...
				$news[] = array(
					...
					'description' => strip_tags($row['description'])== '' ? '一站到底': strip_tags($row['description']),
					'picurl' => $_W['attachurl'] . $row['picture'],
					'url' => create_url('mobile/module/url', array('name' => 'fighting', 'id' => $rid, 'weid' => $_W['weid']))
				);
			...
			if ($content == '退出') {
				...
				if ($_SESSION['num'] == 1) {
					...
					$addjf = pdo_update('fans', $insert2, array (
						'from_user' => $from_user
					));
					$_SESSION['num']++;
	...
	**/
	...
	function getNickName($from_user) {
		...
		$infos = pdo_fetch($sql_infos, array (
			':from_user' => $from_user
		));
