...
class hlmoonModuleProcessor extends WeModuleProcessor {
	public $name = 'hlmoonModuleProcessor';
	...
	public function respond() {
		...
		$weid = $_W['weid'];//当前公众号ID
				...
				$row = pdo_fetch($sql, array(':rid' => $rid));
					...
					//查询是否被屏蔽
					$lists = pdo_fetch("SELECT status,moonnum,zhongjiang FROM ".tablename($this->table_list)." WHERE from_user = '".$from."' and weid = '".$weid."' and rid= '".$rid."' limit 1" );
					...
					if($now >= $row['start_time'] && $now <= $row['end_time']){						
						if ($row['status']==0){
						    ...
						    return $this->respNews(array(
							    ...
							    'Url' => $this->createMobileUrl('moon', array('id' => $rid)),
						    ));
						}
					}else{
