...
require_once './source/modules/public/Classes/PHPExcel.php';
	...
	$list = pdo_fetchall("SELECT * FROM ".tablename($this->table_data)." WHERE rid =:rid  and weid= :weid ".$Where." order by `moontime` desc" , array(':rid' => $rid,':weid'=>$_W['weid']));				
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('抢月饼活动助抢数据_'.$rid);
...
if (empty($uid)){
header('Content-Disposition: attachment;filename="moondata_'.$rid.'.xlsx"');
}else{
header('Content-Disposition: attachment;filename="moondata_'.$rid.'_'.$uid.'.xlsx"');
}
