...
class RedpacketModuleSite extends WeModuleSite
{
        ...
        global $_GPC, $_W;
         ...
         if ($operation == 'post') { // 添加
             ...
             if (checksubmit('submit')) {
                 ...
                 if (! empty($id)) {
                     pdo_update($this->table_redpacket_user, $data, array('id' => $id ));
                 } else {
            ...
            $id = intval($_GPC['id']);
            pdo_delete($this->table_redpacket_user, array('id' => $id)); // 删除用户表
            message('删除成功！', referer(), 'success');
        }
    ...
    public function doMobileFirend()
    {
        ...
        if($openid==$user['openid']){//用户是哥们自己
            ...
            require IA_ROOT . '/source/modules/redpacket/source/MicrobCore.class.php';
            $core = new MicrobCore($_W['account']);
            $jsPars = $core->buildJSAPIParams();
