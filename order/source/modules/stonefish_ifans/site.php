...
class Stonefish_ifansModuleSite extends WeModuleSite {
	...
	public function doWebGetFans() {
		...
		if (checksubmit('submit')) {
			...
			if ($result['total']) {
				...
				message("公众平台返回接口错误. <br />错误代码为：{$result['errcode']}<br />错误信息为: {$result['errmsg']} <br />错误描述为: " . $result['errcode']);
			}
		}
	...
	public function doWebGetallfansinfo(){
		...
		$num = $_GPC['num']+$i;
		if ($num < $total) {
			message("第".$pindex."页粉丝信息同步成功，当前同步成功".$num."个<br />总数".$total."<br />接下来同步第".$nindex."页，请勿关闭此页面",create_url('site/module/Getallfansinfo', array('name' => 'stonefish_ifans','page' => $nindex,'num' => $num)),'success');
		}else{
		...
		if (checksubmit('submit')) {
			...
			if ($_GPC['sendmode'] == '1') {
				...
				foreach ($list as $key => $value) {
					...
					pdo_insert('stonefish_ifans_groupsend',$insert);
				}
	...
	public function doWebMedia(){
		...
		if (!empty($_W['ispost'])) {
			...
			$ree = pdo_fetch("SELECT * FROM ".tablename('stonefish_ifans_medias')." WHERE id = '{$set}'");
			...
			if (pdo_update('stonefish_ifans_medias',array('media_id' => $data['media_id'],'createtime' => $data['createtime']),array('id'=> $set))) {
				exit('success');
			}
		}
		$pindex = max(1, intval($_GPC['page']));
		...
		$list = pdo_fetchall("SELECT * FROM ".tablename('stonefish_ifans_medias')." WHERE weid = '{$_W['weid']}' $condition ORDER BY id DESC LIMIT ".($pindex - 1) * $psize.','.$psize);
		...
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('stonefish_ifans_medias') . " WHERE weid = '{$_W['weid']}' $condition");
		$pager = pagination($total, $pindex, $psize);
	...
	public function doWebMediaPost(){
		...
		if (checksubmit('submit')) {
			...
			if(pdo_insert('stonefish_ifans_medias',$data)){
