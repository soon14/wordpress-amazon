...
class ScratchModuleSite extends WeModuleSite {
	...
	public function doMobileindex() {
		...
		//录入scratch_fans数据
 		...
 		if(empty($id)){
			$id=pdo_fetchcolumn("SELECT id FROM ".tablename("rule")." WHERE weid = :weid  AND module='scratch' ORDER BY `id` DESC limit 1", array(':weid' => $_W['weid']));
  		}
	...
	//验证手机号码领奖
	public function doMobileaccept(){
		global $_GPC, $_W;	
		$id = intval($_GPC['id']);
		$fansID=$_W['fans']['id'];
		$from_user=$_W['fans']['from_user'];
		...
		//获取密码
		$accept_passwrod = pdo_fetchcolumn("SELECT accept_passwrod FROM ".tablename($this->tablename)." WHERE rid = :rid ORDER BY `id` DESC", array(':rid' => $id));		
 		if($password!=$accept_passwrod){
			$data=array(
				'success'=>0,
				'msg'=>'商家验证密码错误，请确认密码',
			);
		}else{
			$award = pdo_fetch("SELECT id,award_sn FROM ".tablename('award')." WHERE rid = ".$id." and fansID=".$fansID." and from_user='".$from_user."'");				
			if($award==false){
				$data=array(
					'success'=>0,
					'msg'=>'保存数据错误！',
				...
				if($_GPC['code']==$award['award_sn']){
					$temp=pdo_update('award', array('accept_satatus'=>1,'accept_time'=>time()),array('id'=>$award['id']));
					//$temp=true;
					if($temp==false){
						$data=array(
							'success'=>0,
