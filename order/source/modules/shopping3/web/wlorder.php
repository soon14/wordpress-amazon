...
if ($operation == 'display') {
	...
	if (checksubmit('submit4') && !empty($_GPC['select'])) {
		pdo_update('shopping3_order', array('ispay'=>1), " id  IN  ('".implode("','", $_GPC['select'])."')");
		message('订单批量确认付款成功！', $this->createWebUrl('order', array('page' => $_GPC['page'])));
	}	
	if (checksubmit('submit5') && !empty($_GPC['select'])) {
		pdo_update('shopping3_order', array('sendstatus' => 1), " id  IN  ('".implode("','", $_GPC['select'])."')");
		message('订单批量确认配送成功！', $this->createWebUrl('order', array('page' => $_GPC['page'])));
	}	
	if($_GPC['confrim']==1){
		if (!empty($_GPC['tel'])) {
