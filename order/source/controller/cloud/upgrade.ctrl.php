...
if(empty($_W['setting']['site']['key']) || empty($_W['setting']['site']['token'])) {
	message("你的程序需要在微服务云服务平台注册你的站点资料, 来接入云平台服务后才能使用在线更新功能.", create_url('cloud/profile'), 'error');
}
...
if($do == 'upgrade') {
	...
	if(!empty($upgrade) && !empty($upgrade['upgrade'])) {
		...
		if(checksubmit('do-download') && $_GPC['hash'] == $hash) {
			...
			$pars['standalone'] = '1';
			$form = '<form id="form1" action="http://addons.weixfu.cn/gateway.php" method="post">';
