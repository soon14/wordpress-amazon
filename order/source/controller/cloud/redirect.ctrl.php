...
$authurl = 'http://addons.weixfu.cn/site.php?act=passport';
...
if($do == 'promotion') {
	if(empty($_W['setting']['site']['key']) || empty($_W['setting']['site']['token'])) {
		message("你的程序需要在微服务云服务平台注册你的站点资料, 来接入云平台服务后才能使用推广功能.", create_url('cloud/profile'), 'error');
	}
...
if($do == 'recommend') {
	if(empty($_W['setting']['site']['key']) || empty($_W['setting']['site']['token'])) {
		message("你的程序需要在微服务云服务平台注册你的站点资料, 来接入云平台服务后才能使用此功能.", create_url('cloud/profile'), 'error');
	}
