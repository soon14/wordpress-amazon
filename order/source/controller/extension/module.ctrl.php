...
$dos = array('installed', 'check', 'prepared','batchinstall', 'install', 'upgrade', 'uninstall', 'cloud', 'designer', 'permission', 'convert');
		...
		if ($handle = opendir($path)) {
			while (false !== ($modulepath = readdir($handle))) {
				...
				if (!is_array($manifest['versions'])){
				  if(DEVELOPMENT) {
				    echo "本模块出错:".$modulepath."</br>";
				    echo "错误信息:".$manifest."</br>";
				  }
				} else {
...
if($do == 'permission') {
	...
	if(empty($module)) {
		message('你访问的模块不存在. 或许你愿意去微服务云服务平台看看. ', 'http://v2.addons.we7.cc/search.php?q=' . $id);
	}
...
if($do == 'batchinstall') {
	...
	$initNG = true;
	$i=0;
	foreach ($localUninstallModules as $key => $value) {
	  $str=installapp($value['name']);
	  if ($str){
        $i++;
	   }
	   if ($i==15)
	...
	message("安装成功".$i."个模块，请继续刷新本页","","success");
...
function batch_manifest_check($id, $m) {
	if(is_string($m)) {
		return false;
...
function installapp($modulename){
  	$modulepath = IA_ROOT . '/source/modules/' . $modulename . '/';
...
class {$name}ModuleReceiver extends WeModuleReceiver {
	public function receive() {
		...
		//这里定义此模块进行消息订阅时的, 消息到达以后的具体处理过程, 请查看微服务文档来编写你的代码
	}
}
TPL;
	}
