			...
			if($ret['error']) {
				switch($ret['error']) {
					case 'error-client':
						...
						break;
					case 'error-warning':
						message('存在错误, 不能自动更新. 错误详情: 你使用的系统是由非法渠道传播的, 微服务已记录这个情况, 泄露此系统的商业用户将被停止商业服务, 微服务团队保留法律追究的权利. (如果属于误报, 请联系微服务团队)');
						break;
					case 'error-limit':
						message('公众开源版用户需要推荐2个用户安装使用微服务系统(需要推荐的用户注册云服务), 才能激活自动更新功能. 您也可以升级为商业用户, 来直接使用自动更新功能. 如果您是微服务合作开发者, 请联系微服务团队直接激活自动更新.', create_url('cloud/promotion'));
						break;
					default:
...
function cloud_m_query() {
	...
	$dat = ihttp_post('http://addons.weixfu.cn/gateway.php', $pars);
...
function cloud_sms_send($mobile, $content) {
	...
	return error(-1, '发送短信失败, 请联系系统管理人员. 错误详情: 不能链接微服务云服务网关');
}
...
function cloud_upgrade_download($archive) {
	...
	$dat = ihttp_request('http://addons.weixfu.cn/gateway.php', $pars, $headers, 300);
	$ret = _cloud_parse_response($dat);
	if(empty($ret)) {
