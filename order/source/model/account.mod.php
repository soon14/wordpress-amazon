<?php
/**
 * [WeEngine System] Copyright (c) 2013 weixfu.cn
 */
...
function getSignPackage($account) {
	...
	$jsapiTicket=account_weixin_jsapi_ticket($account);
...
function account_weixin_jsapi_ticket($account) {
	//by 超级无聊;QQ：40039885
	if(is_array($account['jsapi_ticket']) && !empty($account['jsapi_ticket']['ticket']) && !empty($account['jsapi_ticket']['expire']) && $account['jsapi_ticket']['expire'] > TIMESTAMP&&1==2) {
		...
		$accessToken = account_weixin_token($account);
