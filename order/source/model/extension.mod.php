...
function ext_module_manifest_parse($xml,$d) {
	$dom = new DOMDocument();
	$dom->loadXML($xml);
...
function ext_module_manifest($modulename, $withRemote = false) {
	...
	return ext_module_manifest_parse($xml,$modulename);
}
...
<manifest xmlns="http://www.weixfu.cn" versionCode="{$versions}">
...
function ext_module_manifest_validate_050() {
	$xsd = <<<TPL
<?xml version="1.0" encoding="utf-8"?>
<xs:schema xmlns="http://www.weixfu.cn" xmlns:xs="http://www.w3.org/2001/XMLSchema">
