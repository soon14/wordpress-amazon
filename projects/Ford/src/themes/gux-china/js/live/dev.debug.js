	...
	function AutoNaviAPI(config){
        ...
        */
		function autonaviMap(element, options){
			...
			this.map = new AMap.Map(element, {center: startCenter, level: options.zoom} );
			this.markers = [];
            this.dealers = [];
        ...
        * @param {string} location: the location, eg: "???????????¡¤?¡¤????¨¨???¡¤?¨¨??"
        ...
        */
		function autonaviMarker(m, pos, label, infoWindowContent, callback, images){	
			...
			this.inactiveImage = images && images.inactive ? images.inactive : 'map-marker.png';
			this.activeImage = images && images.active ? images.active : 'map-marker-active.png';
			this.marker = new AMap.Marker({
				map: m.map, 
				position: pos, 
				icon: imagePath + 'map-marker.png', 
				selected: false,
				 content: '<div class="map-marker"><img src="' + imagePath + this.inactiveImage + '"><span class="marker-label">' + label + '</span></div>'
...
/*
Author:                 Joel Wang
File name:              paymentEstimator.js
Description:    		payment estimator module
Dependencies:   		angular.min.js  angular-route.min.js
Usage:                  China Payment Estimator project
*/
var guxApp = guxApp || {};
//financial china credit scope variable
var FCC = FCC || {};

'use strict';
guxApp.paymentEstimator = angular.module('paymentEstimator', ['ngRoute']);
