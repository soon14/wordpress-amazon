...
(function(i){var g=document,j=Array.prototype.push,k=Array.prototype.slice,l=Array.prototype.concat,f=function(a,c){if(arguments.length<2)c=g;return new f.fn.init(a,c)};f.Event=function(a){var c=this;if(!a)a=i.event;c.target=a.target||a.srcElement||document;c.preventDefault=function(){if(a.preventDefault)a.preventDefault();else a.returnValue=false};c.stopPropagation=function(){a.stopPropagation&&a.stopPropagation();a.cancelBubble=true}};f.fn=f.prototype={$ver:1,length:0,push:j,slice:k,concat:l,sort:[].sort,
splice:[].splice,init:function(a,c){var b=this,d,e;j.call(b);if(!a||!c)return b;if(a.$ver)return a;else if(typeof a!="string"){b.push(a);return b}else if(c&&c.$ver){for(d=c.length;d-- >0;)b=b.merge(f(a,c[d]));return b}if(a===g||!(e=c.childNodes)||!a||a.length<1)return b;if(a.charAt(0)=="#"){(a=g.getElementById(a.slice(1)))&&b.push(a);return b}for(d=e.length;d-- >0;)if(a.charAt(0)=="."&&e[d].className&&e[d].className.indexOf(a.slice(1))>=0||e[d].tagName&&e[d].tagName==a.toUpperCase())b.push(e[d]);
else b=b.merge(f(a,e[d]));return b},merge:function(a){for(var c=this,b=c.length,d=0,e=a.length;d<e;d++)c[b++]=a[d];c.length=b;return c}};f.fn.init.prototype=f.fn;f.fn.closest=function(a){var c=this,b=new f.fn.init,d=a.charAt(0),e,h;for(h=c.length;h-- >0;)for(e=c[h];e=e.parentNode;)if(d=="."&&e.className&&e.className.indexOf(a.slice(1))>=0||d=="#"&&e.id&&e.id==a.slice(1)||e.tagName&&e.tagName==a.toUpperCase()){b.push(e);break}return b};f.fn.ready=function(a){function c(){b||a();b=1}var b=0;if(g.addEventListener){g.addEventListener("DOMContentLoaded",
c,false);w.addEventListener("load",c,false)}else if(g.attachEvent){g.attachEvent("onreadystatechange",c);w.attachEvent("onload",c)}};f.fn.bind=function(a,c){var b=this,d,e=function(m){c(new f.Event(m))};for(d=b.length;d-- >0;){var h=b[d];if(g.addEventListener)h.addEventListener(a,e,false);else g.attachEvent&&h.attachEvent("on"+a,e)}};f.fn.addClass=function(a){var c=this,b;for(b=c.length;b-- >0;){var d=c[b];if(d.className.indexOf(a)<0)d.className=d.className+" "+a}};f.fn.removeClass=function(a){var c=
this,b;for(b=c.length;b-- >0;){var d=c[b];if(d.className&&d.className.length>0)d.className=d.className.replace(a,"")}};f.fn.hasClass=function(a){var c=this,b;for(b=c.length;b-- >0;){var d=c[b];if(!d.className||d.className.indexOf(a)<0)return 0}return 1};i.$=f})(window);
...
$(document).ready(function(){var c=$("#cityDropdownData");if(c.length>0){c=JSON.parse(c[0].innerHTML);ND.selectOption.init(c)}var f=$("#menuSubmition",d)[0],g=$("#navurl",d)[0];f.onsubmit=function(){if(f.action.length<2)return false};g.onchange=function(){f.action=g.options[g.selectedIndex].value};var h=$("IMG",d.getElementById("imgnav"))[0];c=$("A",d.getElementById("colors"));h&&c&&c.bind("click",function(a){a.preventDefault();a=$(a.target).closest("a")[0];var b=$(".colortxt",$(a).closest(".color"))[0];
if(a&&b){h.src=a.href;b.innerHTML=a.title}});var j=$("#fullarticle"),k=$(".hide");$("A",j[0]).bind("click",function(a){a.preventDefault();j.addClass("hide");k.removeClass("hide");k.hasClass("alert")&&$(".content").addClass("hide")});c=$("a",$("#share"));c.length&&ND.share.init(c,document.referrer);$("#phonelist").bind("change",function(a){var b=a.target.value,e;if(b!="-1"){for(i=0;i<a.target.length;i++){e="#"+a.target[i].value;$(e).addClass("bq_hide")}$("#"+b).removeClass("bq_hide")}else for(i=0;i<
a.target.length;i++){e="#"+a.target[i].value;$(e).removeClass("bq_hide")}});$(".bq_prev").bind("click",function(a){var b=Number(a.target.parentNode.parentNode.id.substring(1));a=$("#p"+b);b=Number(b-1);var e=$("#p"+b);try{document.getElementById("p"+b);a.removeClass("bq_show");a.addClass("bq_hide");e.removeClass("bq_hide");e.addClass("bq_show")}catch(l){}});$(".bq_next").bind("click",function(a){var b=Number(a.target.parentNode.parentNode.id.substring(1));a=$("#p"+b);b=Number(b+1);var e=$("#p"+b);
try{document.getElementById("p"+b);a.removeClass("bq_show");a.addClass("bq_hide");e.removeClass("bq_hide");e.addClass("bq_show")}catch(l){}})});
