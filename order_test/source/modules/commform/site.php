    ...
    public function doMobileshowform() {
		...
		if(checksubmit('submit')){
	        ...
	        if(pdo_insert($tabname,$insert)){
               $form['successtip']=empty($form['successtip'])?"提交成功":$form['successtip'];
	          ...
	          $form['failtip']=empty($form['failtip'])?"提交失败":$form['failtip'];    
	       		message($form['failtip'],create_url('mobile/module',array('do' => 'showform','weid'=>empty($_GPC['__weid'])?$_GPC['weid']:$_GPC['__weid'],'userid'=>$userid,'name' => 'commform',fid=>$id,fn=>$fn)), 'error');
	        }
		}
