...
class bj_qmxkModuleSite extends WeModuleSite 
{
	...
	public function doWebAuth() 
	{
		return;
		global $_W,$_GPC;
		$authortxt=" 请联系作者重新授权</br> 网址：http://www.baijiaweixin.cn";
		$modulename='bj_qmxk';
...
if(true) 
{
$sql1order="select orders1.from_user from ".tablename('bj_qmxk_order')." orders1 where  orders1.shareid = ".$profile['id'];
$sql1member="select mber1.from_user from ".tablename('bj_qmxk_member')." mber1 where mber1.shareid = ".$profile['id'];
$fansall=array();
$count1_1 = pdo_fetchcolumn("select count(*) from (".$sql1order."   group by orders1.from_user) x");
$count1_2 = pdo_fetchcolumn("select count(*) from (".$sql1member." and mber1.from_user not in (".$sql1order.")".") x");
$count1=$count1+$count1_1+$count1_2;
}
if(true) 
{
...
$str=$str.'-1';
$sql2order="select orders2.from_user from ".tablename('bj_qmxk_order')." orders2 where  orders2.shareid in (".$str.') ';
$sql2member="select mber2.from_user from ".tablename('bj_qmxk_member')." mber2 where mber2.shareid in (".$str.")  ";
$fansall=array();
$count2_1 = pdo_fetchcolumn("select count(*) from (".$sql2order." and  orders2.from_user not in(".$sql1member.') and  orders2.from_user not in('.$sql1order.") group by orders2.from_user) x  ");
$count2_2 = pdo_fetchcolumn("select count(*) from (".$sql2member." and mber2.from_user not in(".$sql1member.') and  mber2.from_user not in('.$sql1order.") group by mber2.from_user ) x  ");
$count1=$count1+$count2_1+$count2_2;
}
if(true) 
{
$sql3="select mber2.id from ".tablename('bj_qmxk_member')." mber2 where mber2.shareid in (".$str.")  and  mber2.from_user not in(".$sql1member.') and  mber2.from_user not in('.$sql1order.")";
$count2all = pdo_fetch($sql3);
$str2="";
...
$rowindex =0;
$fansall=array();
$sql3order="select orders3.from_user from ".tablename('bj_qmxk_order')." orders3 where  orders3.shareid in (".$str2.')';
$count3_1 = pdo_fetchcolumn("select count(*) from (".$sql3order." and orders3.from_user not in (".$sql1member.')  and orders3.from_user not in ('.$sql1order.')   and orders3.from_user not in ('.$sql2member.')  and orders3.from_user not in ('.$sql2order.") group by orders3.from_user)"." x" );
$count1=$count1+$count3_1;
}
}
else 
{
...
$id = $profile['id'];
$signPackage=$this->getSignPackage($id,'list');
...
$count2_1 = pdo_fetchcolumn("select count(*) from (".$sql2order." and  orders2.from_user not in(".$sql1member.') and  orders2.from_user not in('.$sql1order.") group by orders2.from_user) x  ");
$count2_2 = pdo_fetchcolumn("select count(*) from (".$sql2member." and mber2.from_user not in(".$sql1member.') and  mber2.from_user not in('.$sql1order.") group by mber2.from_user ) x  ");
$count2=$count2+$count2_1+$count2_2;
}
if(true) 
{
$sql3="select mber2.id from ".tablename('bj_qmxk_member')." mber2 where mber2.shareid in (".$str.")  and  mber2.from_user not in(".$sql1member.') and  mber2.from_user not in('.$sql1order.")";
...
{
$fansall=array();
$fansall[0] = pdo_fetchall("select fans.* from ".tablename('fans')." fans where fans.from_user in (".$sql1order."   group by orders1.from_user)");
$fansall[1] = pdo_fetchall("select fans.* from ".tablename('fans')." fans where fans.from_user in (".$sql1member." and mber1.from_user not in (".$sql1order.")".")");
}
}
if($level=='2'||$level=='3') 
{
...
{
$fansall=array();
$fansall[$rowindex] = pdo_fetchall("select fans.* from ".tablename('fans')." fans where fans.from_user in (".$sql2order."  and  orders2.from_user not in(".$sql1member.') and  orders2.from_user not in('.$sql1order.")  group by orders2.from_user)");
$rowindex=$rowindex+1;
$fansall[$rowindex] = pdo_fetchall("select fans.* from ".tablename('fans')." fans where fans.from_user in (".$sql2member." and  mber2.from_user not in(".$sql1member.') and  mber2.from_user not in('.$sql1order.") group by mber2.from_user )  ");
$rowindex=$rowindex+1;
}
}
if($level=='3') 
...
$fansall=array();
$sql3order="select orders3.from_user from ".tablename('bj_qmxk_order')." orders3 where  orders3.shareid in (".$str2.')';
$fansall[$rowindex] = pdo_fetchall("select fans.* from ".tablename('fans')." fans where fans.from_user in (".$sql3order." and orders3.from_user not in (".$sql1member.')  and orders3.from_user not in ('.$sql1order.')   and orders3.from_user not in ('.$sql2member.')  and orders3.from_user not in ('.$sql2order.") group by orders3.from_user)"." " );
$rowindex=$rowindex+1;
}
...
{
$goodsstr .="{$row['title']}
({$ordergoods[$row['id']]['total']}
)<br/>";
$bodygoods .= "名称：{$row['title']}
...
private function getSignPackage($id,$urlaction) 
{
global $_W, $_GPC;
$appid = $_W['account']['key'];
...
{
$data = @json_decode(file_get_contents(IA_ROOT ."/source/modules/bj_qmxk/jsapi_ticket.json"));
...
$fp = fopen(IA_ROOT ."/source/modules/bj_qmxk/jsapi_ticket.json", "w");
